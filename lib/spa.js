...
class Spa extends EventEmitter {
  ...
  setJetState(jet, state) {
    let self = this;
    if (!self.jetTargets) self.jetTargets = {};
    if (!self.jetBusy) self.jetBusy = {};
    // Latest command wins: a running press loop picks the new target up, so the
    // paired on+speed commands HA sends for one fan action collapse into a
    // single press sequence instead of racing each other
    self.jetTargets[jet] = state;
    if (self.jetBusy[jet]) {
      logDebug(`Jet ${jet} press loop already running, target updated to ${state}`);
      return;
    }
    self.jetBusy[jet] = true;
    self.jetPressLoop(jet)
      .catch(err => logError(`Failed to set jet ${jet} state ${state}: ${err}`))
      .finally(() => { self.jetBusy[jet] = false; });
  }

  async jetPressLoop(jet) {
    // The ControlMySpa backend treats every command as a jets-button press that
    // cycles OFF -> HIGH -> LOW -> OFF regardless of the requested state, so
    // press until the pump reports the target speed. Before each extra press,
    // re-read the spa state once more: the gateway can be slow to report and a
    // stale read here would trigger a phantom press that overshoots the target.
    const maxPresses = 4;
    for (let press = 1; press <= maxPresses; press++) {
      if (this.getPumps()[jet].value === this.jetTargets[jet]) break;
      let res = await this.client.setComponentState("jet", jet, this.jetTargets[jet]);
      if (!res) {
        setTimeout(() => this.updateSpa(), this.refreshAfterUpdate);
        return;
      }
      this.emit('status_updated');
      if (this.getPumps()[jet].value === this.jetTargets[jet]) continue;
      await new Promise(resolve => setTimeout(resolve, 3000));
      await this.client.getSpa();
      this.emit('status_updated');
      let cur = this.getPumps()[jet].value;
      if (cur !== this.jetTargets[jet] && press < maxPresses) {
        logInfo(`Jet ${jet} is ${cur}, wanted ${this.jetTargets[jet]}; pressing again (${press + 1}/${maxPresses})`);
      }
    }
    if (this.getPumps()[jet].value !== this.jetTargets[jet]) {
      logError(`Jet ${jet} did not reach ${this.jetTargets[jet]} after ${maxPresses} presses`);
      setTimeout(() => this.updateSpa(), this.refreshAfterUpdate);
    }
  }
