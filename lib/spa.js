...
class Spa extends EventEmitter {
  ...
  setJetState(jet, state, attempt = 0) {
    let self = this;
    self.client.setComponentState("jet", jet, state).then(res => {
      ...
      if (res && curState === state) {
        ...
        // The ControlMySpa backend treats the command as a jets-button press
        // that cycles OFF -> LOW -> HIGH -> OFF regardless of the requested
        // state, so press again until the pump reaches the requested state
        logInfo(`Jet ${jet} is ${curState}, wanted ${state}; pressing again (${attempt + 2}/3)`)
        this.emit('status_updated')
        self.setJetState(jet, state, attempt + 1);
      } else {
        setTimeout(() => self.updateSpa(), self.refreshAfterUpdate);
      }
